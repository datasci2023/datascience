...
class MetadataProcessor(
    object
):  # the object has to be updated to Processor when we'll put everything together
    ...
    def uploadData(self, path):
        with connect(
            self.dbPathOrUrl
        ) as con:  # Insert in the parenthesis the name of the file containing the database; the first time, if the database corresponding to that name/path doesn't exist, a database with that name is created; it should be a database separate from the triplestore for now
            metadata = read_csv(
                "E:\\Documents\\GitHub\\datascience\\metadata.csv",
            ...
            collections.insert(
                0, "collectionId", Series(collections_internal_id, dtype="string")
            ...
            canvases.insert(
                0, "canvasesId", Series(canvases_internal_id, dtype="string")
            )
            ...
            collection_manifest.insert(
                0, "collection_id", Series(col_id, dtype="string")
            )
            ...
            collection_manifest.to_sql(
                "Collection-Manifest", con, if_exists="replace", index=False
            )
            manifest_canvas.to_sql(
                "Manifest-Canvas", con, if_exists="replace", index=False
            )
