...
#         with connect(self.dbPathOrUrl) as con:  
            # Insert in the parenthesis the name of the file containing the database; the first time, if the database corresponding to that name/path doesn't exist, a database with that name is created; it should be a database separate from the triplestore for now
with connect("relationaldatabase.db") as con:

    metadata = read_csv("C:\\Users\\chiar\\Documents\\GitHub\\datascience\\metadata.csv", 
    ...
    collections.insert(0, "id", Series(collections_id, dtype="string"))  
    ...
    for idx, row in collections.iterrows():
        collections_internal_id.append("collection-" + str(idx)) 
        # Create a list of internal ids

    collections.insert(0, "collectionId", Series(collections_internal_id, dtype="string"))  
    ...
    manifests.insert(0, "id", Series(manifests_id, dtype="string"))
    ...
    for idx, row in manifests.iterrows():
        manifests_internal_id.append("manifest-" + str(idx))
        # Create a list of internal ids

    manifests.insert(0, "manifestId", Series(manifests_internal_id, dtype="string"))
